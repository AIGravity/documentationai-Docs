...
The index route is defined in `apps/web/app/routes/_index.tsx`. It exports Remix metadata through a typed arrow function and returns `Welcome`.
...
The `/home` route is defined in `apps/web/app/routes/home.tsx`. It is effectively identical to the index route, uses the same typed arrow-function metadata export, and also renders `Welcome`.
