...
This page reflects the current implementation in the repository, not an intended future configuration model. If a setting is listed here, it was found in source files, Dockerfiles, package scripts, or GitHub Actions workflows. The API and AI services now also read environment variables for agent ranking, LLM provider configuration, and client-side environment detection.
...
The repository now consumes application environment variables in both the API and AI services, and the web build injects a client-side environment identifier during CI and container builds.

<Callout kind="info">

The API service uses NestJS ConfigModule as a global configuration layer with `.env` file support. The AI service reads LLM provider settings from environment variables, including the OpenRouter API key, base URL, and verification timeout.
...
<ParamField body="repo-defined-app-env" param-type="supported" required="true" show-location="false">

**Purpose:** Configure application runtime through repository-defined environment variables.

**Type:** environment-backed application configuration.

**Explicit value:** present in the API, AI service, and web build pipeline.

**Required:** Yes for the current environment-driven paths. The API uses environment variables for agent ranker configuration, the AI service uses them for LLM provider settings, and the web build injects a client-side environment identifier.


</ParamField>

<ParamField body="env-file-support" param-type="supported" required="true" show-location="false">

**Purpose:** Load local configuration from `.env` files.

**Type:** `.env` file support.

**Explicit value:** implemented through NestJS ConfigModule in the API service.

**Required:** No. `.env` loading is supported for local and deployed API configuration, but individual variables remain required only where the consuming feature depends on them.


</ParamField>

<ParamField body="AGENT_RANKER_MIN_SCORE" param-type="string" required="true" show-location="false">

**Purpose:** Define the minimum quality score threshold for agent ranker acceptance in the API service.

**Type:** string.

**Explicit value:** environment-defined at runtime.

**Required:** Yes where agent ranker evaluation is enabled. Set in GitHub Actions from `vars.API_AGENT_RANKER_MIN_SCORE` and also configured in the ECS task definition.


</ParamField>

<ParamField body="AGENT_RANKER_API_URL" param-type="string" required="true" show-location="false">

**Purpose:** Define the API service URL used for agent ranker scoring requests.

**Type:** string.

**Explicit value:** environment-defined at runtime.

**Required:** Yes where agent ranker scoring is enabled. Set in GitHub Actions from `vars.API_AGENT_RANKER_API_URL` and also configured in the ECS task definition.


</ParamField>

<ParamField body="VITE_APP_ENVIRONMENT" param-type="string" required="true" show-location="false">

**Purpose:** Define the client-side environment identifier for the web service.

**Type:** string.

**Explicit value:** environment-defined at build and runtime in the container image.

**Required:** Yes for the current CI and container build path. Passed as both Docker build argument and container environment variable, and set in GitHub Actions from `vars.WEB_VITE_APP_ENVIRONMENT`.


</ParamField>

<ParamField body="OPENROUTER_API_KEY" param-type="string" required="true" show-location="false">

**Purpose:** Authenticate AI service requests to the OpenRouter LLM provider used by category and description generators.

**Type:** string.

**Explicit value:** environment-defined at runtime.

**Required:** Yes for AI generation flows that call OpenRouter.


</ParamField>

<ParamField body="OPENROUTER_BASE_URL" param-type="string" required="false" show-location="false">

**Purpose:** Override the base URL for OpenRouter API requests in the AI service.

**Type:** string.

**Explicit value:** defaults to `https://openrouter.ai/api/v1`.

**Required:** No. The AI service uses the default OpenRouter base URL when this variable is not set.


</ParamField>

<ParamField body="VERIFICATION_REQUEST_TIMEOUT_SECS" param-type="string" required="false" show-location="false">

**Purpose:** Define the hard timeout for verification requests in the AI service.

**Type:** string.

**Explicit value:** defaults to `10`.

**Required:** No. The AI service uses the default timeout when this variable is not set.
...
The repository also uses environment variables outside application code in Docker and GitHub Actions.
...
#### Application environment variables in CI/CD

<ParamField body="API_AGENT_RANKER_MIN_SCORE" param-type="GitHub Actions variable" required="true" show-location="false">

**Purpose:** Provide the CI source value for `AGENT_RANKER_MIN_SCORE` in API build and deployment workflows.

**Type:** repository or environment variable.

**Explicit value:** `vars.API_AGENT_RANKER_MIN_SCORE`.

**Required:** Yes where the workflow injects agent ranker settings into deployed API configuration.


</ParamField>

<ParamField body="API_AGENT_RANKER_API_URL" param-type="GitHub Actions variable" required="true" show-location="false">

**Purpose:** Provide the CI source value for `AGENT_RANKER_API_URL` in API build and deployment workflows.

**Type:** repository or environment variable.

**Explicit value:** `vars.API_AGENT_RANKER_API_URL`.
...
<ParamField body="WEB_VITE_APP_ENVIRONMENT" param-type="GitHub Actions variable" required="true" show-location="false">

**Purpose:** Provide the CI source value for `VITE_APP_ENVIRONMENT` in web build workflows.

**Type:** repository or environment variable.

**Explicit value:** `vars.WEB_VITE_APP_ENVIRONMENT`.

**Required:** Yes for the current web image build path that passes the value as both Docker build argument and environment variable.
...
**Purpose:** Call out that the API service has moved from fully hardcoded configuration to partial environment-driven configuration through ConfigModule, while ports, CORS origin, and several deployment choices remain hardcoded.

**Type:** implementation characteristic.

**Explicit value:** present across runtime code, application environment variables, and workflows.
