...
The current implementation does not define route-specific error handling for `GET /health`, so mismatches use FastAPI and Starlette's default responses.

- **`404 Not Found`** — Returned when the request path does not match a defined route, such as `/api/health`.
- **`405 Method Not Allowed`** — Returned when the path matches `/health` but the HTTP method is not supported, such as `POST /health`.
- **`500 Internal Server Error`** — Possible only if the service or runtime encounters an unexpected failure while handling the request.
