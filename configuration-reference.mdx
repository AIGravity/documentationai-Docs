...
This repository still relies far more on static configuration than on environment-driven configuration, but the API service now reads Razorpay environment variables for payment setup. Those variables gate the bounty pay-to-post flow and the related payment verification paths, making payment configuration the main exception to the otherwise hardcoded runtime model.
...
- **Application runtime code** reads Razorpay environment variables that control whether bounty payment is required and how payment verification is handled.
...
The API service reads three Razorpay environment variables for payment configuration. Together, these variables determine whether bounty payment is enabled, whether payment orders can be created, and whether incoming payment events can be verified.

### Current status

The current application environment variable surface is limited to the three Razorpay settings below.

<ParamField body="RAZORPAY_KEY_ID" param-type="string" required="false" show-location="false">

**Purpose:** Razorpay public key ID used to create payment orders and passed to the client checkout. When set, bounty payment is enabled and bounties with a prize amount require payment before going live. When unset, bounties are created as open immediately, which supports free or development mode.

**Type:** string.

**Required:** Yes for bounty pay-to-post and credits payment flows. When not set, payment is disabled and bounties are free.


</ParamField>

<ParamField body="RAZORPAY_KEY_SECRET" param-type="string" required="false" show-location="false">

**Purpose:** Razorpay secret key used for order creation and checkout signature verification on the server side.

**Type:** string.

**Required:** Yes when `RAZORPAY_KEY_ID` is set.


</ParamField>

<ParamField body="RAZORPAY_WEBHOOK_SECRET" param-type="string" required="false" show-location="false">

**Purpose:** Secret used to verify incoming Razorpay webhook signatures. The webhook handler now routes both credits and bounty payment events.

**Type:** string.

**Required:** Yes for webhook-based payment confirmation.
...
`RAZORPAY_KEY_ID` currently acts as a de facto feature flag for the bounty pay-to-post flow. When it is set, bounties with a prize amount are created in `pending_payment` status and require payment before going live. When it is absent, bounties are created as `open` immediately.
...
**Purpose:** Call out that ports, CORS origin, and several deployment choices remain hardcoded, while payment configuration now uses environment variables as a limited exception.

**Type:** implementation characteristic.

**Explicit value:** present across runtime code and workflows, with Razorpay payment settings as the current app-level environment-driven path.
