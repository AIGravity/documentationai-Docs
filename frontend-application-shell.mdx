...
The frontend is a Remix application with multiple route modules, builder and admin interfaces, agent customization flows, API client integration, shared components, and backend-driven data flows. It establishes the document structure, asset pipeline, runtime boundaries, product-specific views, and application interfaces for the web app.

## App purpose and current scope

The frontend now provides product-specific screens and workflows rather than serving only as a starter shell. Current application areas include builder dashboards, admin dashboards, agent customization flows, and API-backed application interfaces.

<Callout kind="info">

This page reflects the current implementation in `apps/web`. The frontend includes routing, layout, styling, error handling, local scripts, Docker packaging, product-specific screens, API client integration, and backend data flows.
...
The frontend includes multiple route modules. The original index and home routes remain part of the application, alongside builder dashboard routes, admin dashboard routes, agent customization routes, and marketplace-related routes.

Product-specific routes use Remix loaders and actions to fetch and mutate application data through the API. This separates route-level data loading and mutations from the views that render each application area.

### `/`

The index route is defined in `apps/web/app/routes/_index.tsx`. It exports Remix metadata and returns `Welcome`. This route is an example of the original application shell and now exists alongside product-specific routes.
...
The `/home` route is defined in `apps/web/app/routes/home.tsx`. It remains effectively identical to the index route and also renders `Welcome`.
...
The index and home routes preserve the original shell behavior. Builder, admin, agent customization, and marketplace-related routes provide the product-specific route structure and backend data flows.

## Root layout

`apps/web/app/root.tsx` defines the application document shell. It imports global CSS, registers font-related links, provides the HTML document structure, and renders the route outlet. The root shell continues to provide the document structure while route modules compose product-specific views.
...
- `Outlet` is the only content rendered by the root app component, so route modules compose the product-specific views rendered within the document shell.
...
The visible UI for the original index and home routes lives in `apps/web/app/welcome/welcome.tsx`. It remains a placeholder screen and historical example of the initial shell, but it is no longer the only visible UI in the frontend.
...
The broader frontend now includes builder interfaces, admin dashboards, agent customization flows, marketplace-related views, and other product-specific components. These interfaces use route modules and API clients to support backend-driven application behavior.
...
The original unit tests have not been written yet. Some legacy starter artifacts may remain, and parts of the frontend may still need consolidation around shared styling and component patterns.

</Callout>

These limitations affect ongoing frontend work, but the application now includes product-specific routes, dashboards, customization flows, API client integration, and backend-driven interfaces.
