...
The `GET /health` route does not define custom non-200 responses. Non-200 behavior comes from FastAPI defaults or unexpected runtime failures rather than endpoint-specific error handling.

- **`404 Not Found`** — FastAPI may return this when you request a path that is not defined.
- **`405 Method Not Allowed`** — FastAPI may return this when you call `/health` with an unsupported HTTP method.
