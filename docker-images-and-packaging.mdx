...
### Amazon ECR publishing through the deployment workflow

The new-account deployment workflow provides a second publishing path. It builds the same service Dockerfiles with each service directory as the Docker build context, then pushes the resulting images to Amazon ECR instead of GitHub Container Registry.

ECR image names follow the `gravity-{service}:{tag}` pattern within the configured ECR registry. The workflow selects the service image name and tag when it builds and pushes each image:
...
- name: Build and push image
  ...
  with:
    ...
    push: true
    tags: ${{ env.ECR_REGISTRY }}/gravity-${{ matrix.service }}:${{ needs.version.outputs.version }}
```

For web builds, the deployment workflow also passes Vite and Sentry values as Docker build arguments. These values are available during the image build alongside the service-specific context:
...
The web Dockerfile does not currently declare `ARG` instructions for the Sentry values. The deployment workflow passes `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, and `SENTRY_PROJECT`, but the Dockerfile does not consume them.

</Callout>

See the [CI/CD Pipelines](/ci-cd) page for the complete deployment workflow, authentication, and environment configuration details.
