...
All three workflows run on `push` and `pull_request` for the `main` and `dev` branches. Each workflow also applies path filters, which keep runs scoped to the service that changed and, for web and API, shared package changes that can affect those builds.
