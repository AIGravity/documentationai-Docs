...
The AI service reads one application environment variable at runtime: `SEARCH_VECTOR_DIST_THRESHOLD`, which controls the vector distance threshold for search and recommendation filtering. The web and API services do not read application environment variables.

<Callout kind="info">

No `.env` files were found in the repository tree. The AI service reads `SEARCH_VECTOR_DIST_THRESHOLD` from the runtime environment with a hardcoded fallback default, not through a dotenv loader.
...
**Purpose:** Control the maximum cosine distance for vector similarity filtering in AI service search and recommendation results. Candidates whose vector distance from the query exceeds this threshold are discarded. A lower value produces stricter filtering with fewer, more relevant results; a higher value increases recall but may include looser matches.

**Type:** float (cosine distance, 0.0–1.0).

**Explicit value:** `0.30`.

**Required:** No. Defaults to `0.30` when the environment variable is not set. Overridable at deploy time via the `AI_SEARCH_VECTOR_DIST_THRESHOLD` GitHub repository variable.
...
<ParamField body="AI_SEARCH_VECTOR_DIST_THRESHOLD" param-type="float" required="false" show-location="false">

**Purpose:** Supply the AI service vector distance threshold at deploy time so it can be tuned without a code change or image rebuild.

**Type:** float (GitHub repository variable).

**Explicit value:** `0.3` (configured in repository variables, not in workflow files).

**Required:** No. When absent, the AI service falls back to its hardcoded default of `0.30`.
...
**Purpose:** Call out that ports, CORS origin, and most deployment choices are hardcoded rather than abstracted into application configuration. The AI service search threshold (`SEARCH_VECTOR_DIST_THRESHOLD`) is the one exception — it is environment-overridable and tunable at deploy time via the `AI_SEARCH_VECTOR_DIST_THRESHOLD` GitHub repository variable.
