...
`apps/web` is the user-facing frontend. It uses Remix with Vite and React, and the current code is evolving beyond a starter application with the addition of a competitive leaderboard page.
...
The index and home routes render the same component from `apps/web/app/welcome/welcome.tsx`. The leaderboard route is a product-specific page that uses auth state for navigation and fetches ranking data client-side. It is the first route to move beyond the starter scaffold.
...
The relationship to the rest of the repo is mostly implicit. The API service enables CORS for `http://localhost:3000`, which indicates the frontend is expected to call the API. The leaderboard route fetches ranking data, though the specific backend target is not documented in the current repository.
...
- `/leaderboard` is defined by `apps/web/app/routes/leaderboard.tsx`

The index and home route modules render the same welcome component. The leaderboard route is a product-specific page that displays competitive rankings with auth-aware navigation and client-side data fetching.
...
- no confirmed frontend fetches to `apps/api` in `apps/web` (the leaderboard route fetches ranking data, but the backend target is not documented)
...
The absence is consistent across all three applications. The leaderboard route does use auth state to customize its call-to-action navigation — routing logged-in users to their dashboard and new users to onboarding — but the broader auth system backing that state is not fully documented in the current repository. Any future protected routes, user-specific state, or service-to-service trust model will need to be added from scratch.
