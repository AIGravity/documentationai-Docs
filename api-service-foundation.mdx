...
The API service in `apps/api` has grown beyond its initial foundation and now includes authentication through Supabase JWT guards, class-validator DTOs, Swagger UI, a global `ValidationPipe`, an exception filter, and many controllers covering auth, agents, marketplace, bounties, payments, and more.

<Callout kind="info">

The service now exposes many controllers and endpoints. Refer to the API reference for the complete endpoint inventory.

</Callout>

## Bootstrap flow

`apps/api/src/main.ts` creates the Nest application from `AppModule`, enables CORS, and starts the HTTP server. The entrypoint also configures request validation, exception handling, observability, API documentation, request-body limits, and raw request bodies for webhook signature validation.
...
// - Enables credentials
// - Configures a global ValidationPipe with whitelist, forbidNonWhitelisted, and transform
// - Configures a global HttpExceptionFilter
// - Configures a SentryUserInterceptor
// - Serves Swagger UI at /api/docs
// - Sets a 10 MB JSON body limit
// - Sets a 1 MB URL-encoded body limit
// - Preserves raw request bodies for webhook signature validation
...
The bootstrap configures a global `ValidationPipe`, `HttpExceptionFilter`, and `SentryUserInterceptor` before the server starts.
...
`AppModule` now wires many feature modules and controllers, including authentication, agents, marketplace, bounties, payments, and additional application domains.

### Current module graph

The module graph includes many feature modules, providers, guards, DTOs, and controllers. `AppModule` serves as the root that connects these modules into the running API.

## Health endpoint

The service exposes `GET /health` as a basic liveness endpoint. It returns the current service status, service identifier, and an ISO 8601 timestamp.

<CodeGroup tabs="Health response,Root response">
...
The root `GET /` endpoint returns the same status and service fields without a timestamp.

### Route behavior

The `/health` route is exposed through the health controller, and there is no global prefix configured in `main.ts`. Clients call the endpoint at `/health`, not `/api/health` or `/v1/health`.

The root `GET /` endpoint provides a second basic availability response for the service.

### Response shape

The health response contains three fields.
...
Returns `api` for this service.
</ResponseField>

<ResponseField name="timestamp" field-type="string" required="true">
Returns the response time as an ISO 8601 timestamp.
</ResponseField>

The root response contains `status` and `service` and does not include `timestamp`.

### What the endpoint does not include

The health endpoint provides a basic liveness check. It does not test downstream dependencies such as databases, external services, queues, or payment providers.
...
The service has grown significantly from its initial foundation. The remaining limitations include:
...
The implementation now supports a broad API surface, authentication, validation, exception handling, Swagger documentation, domain modules, health checks, container packaging, and CI publishing. The next layer of work should resolve the remaining runtime and delivery mismatches, especially the fixed port configuration, Docker port exposure, likely built-entry path mismatch between `main` and `start`, and the missing CI test and lint steps.