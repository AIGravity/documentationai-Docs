...
This repository relies primarily on static configuration, with a small set of environment-driven settings for the agent quality scoring and review-gating pipeline. An `.env.example` file defines optional variables that control agent-ranker behavior at review-submission time. Most other runtime settings — ports, CORS origin, and build configuration — remain hardcoded in source or workflow files.
...
The API service reads two optional environment variables from `.env.example` to control agent quality scoring and review-gated publishing.

<Callout kind="info">

Both variables are optional and ship blank by default. When unset, the quality scoring threshold is disabled and agents always enter the admin review queue.
...
**Purpose:** Set the minimum quality score (0–100) an agent must reach at review-submission time to enter the admin review queue. Agents scoring below the threshold are automatically rejected.

**Type:** number.

**Explicit value:** blank (unset by default — threshold disabled).

**Required:** No. When unset or blank, no quality threshold is enforced and all submissions enter the admin review queue regardless of score.
...
**Purpose:** Define the base URL of the agent-ranker scoring service. When set, the API service contacts the ranker at review-submission time to score the agent against its pre-submit test run output.

**Type:** string (URL).

**Explicit value:** blank (unset by default — scoring is skipped entirely).

**Required:** No. When unset, the API service skips quality scoring and all submissions go directly to the admin review queue.
...
Auto-rejection fires only when **all four** conditions are true simultaneously:

- A test run ID was submitted with the review request
- The ranker service responded with a score (was not down or unreachable)
- The agent is eligible for scoring (at least one use case matched)
- `AGENT_RANKER_MIN_SCORE` is set to a numeric value and the agent's score is strictly below it

If any condition is unmet — for example, the ranker service is unreachable, no test run ID was provided, or the threshold is not configured — the submission always enters the admin review queue and is never auto-rejected.

### Current status

<ParamField body="repo-defined-app-env" param-type="string" required="false" show-location="false">

**Purpose:** Configure agent quality scoring and review-gating behavior through repository-defined environment variables.

**Type:** two optional string/number variables.

**Explicit value:** `AGENT_RANKER_MIN_SCORE` and `AGENT_RANKER_API_URL` in `.env.example`, both blank by default.

**Required:** No. The API service starts without these variables set; quality scoring and auto-rejection are disabled until they are configured.


</ParamField>

<ParamField body="env-file-support" param-type="file" required="false" show-location="false">

**Purpose:** Load local configuration from `.env` files.

**Type:** `.env.example` template.

**Explicit value:** present in the API service directory with two optional variables.

**Required:** No, but copying `.env.example` to `.env` and setting the ranker variables is required to enable quality scoring in local development.
