...
This repository currently relies far more on static configuration than on environment-driven configuration. The inspected implementation does not define any `.env` files, does not implement an app-level feature flag system, and reads environment variables primarily for Sentry observability configuration in the web and API services.
...
- **Sentry entry points** in the web and API services define error monitoring, performance tracing, and trace propagation configuration.
...
The web and API services read environment variables for Sentry observability configuration. No `.env` files are committed to the repository, so these variables must be provided at build or runtime through the hosting environment.

<Callout kind="info">

No `.env` or `.env.example` files were found in the repository tree. The Sentry-related variables below are read from the runtime environment and are not required for local development — Sentry only activates when the environment is set to `production`.
...
#### Web service Sentry environment variables

<ParamField body="VITE_SENTRY_DSN" param-type="string" required="false" show-location="false">

**Purpose:** Sentry Data Source Name for the browser-side SDK.

**Type:** string (Sentry DSN URL).

**Required:** No. When absent, Sentry does not initialize on the client.
...
<ParamField body="VITE_SENTRY_ENVIRONMENT" param-type="string" required="false" show-location="false">

**Purpose:** Identify the deployment environment for Sentry event tagging.

**Type:** string.

**Explicit default:** `development`.

**Required:** No. Sentry only activates when this is set to `production`.
...
<ParamField body="VITE_API_URL" param-type="string" required="false" show-location="false">

**Purpose:** Target URL for cross-origin Sentry trace header propagation.

**Type:** string (URL).

**Required:** No. When set, trace headers are propagated to this URL in addition to same-origin `/api/` requests. When absent, only same-origin traces are propagated.
...
#### API service Sentry environment variables

<ParamField body="SENTRY_DSN" param-type="string" required="false" show-location="false">

**Purpose:** Sentry Data Source Name for the API service SDK.

**Type:** string (Sentry DSN URL).

**Required:** No. When absent, Sentry does not initialize on the API service.
...
<ParamField body="SENTRY_ENVIRONMENT" param-type="string" required="false" show-location="false">

**Purpose:** Identify the deployment environment for Sentry event tagging on the API service.
...
<ParamField body="SENTRY_TRACES_SAMPLE_RATE" param-type="number" required="false" show-location="false">

**Purpose:** Control the fraction of API transactions that Sentry captures for performance tracing.

**Type:** number (0 to 1).

**Explicit default:** `1.0` (100% of transactions).

**Required:** No. Override to reduce transaction volume if Sentry quotas are a concern.
...
Sentry is configured in the web app's browser and server entry points and in the API service bootstrap. It activates only when the environment is set to `production`.

<ParamField body="tracesSampleRate" param-type="number" required="true" show-location="false">

**Purpose:** Control the fraction of transactions that Sentry captures for performance tracing.

**Type:** number (0 to 1).

**Explicit value:** `1.0` (100% of transactions on both client and server).

**Required:** Yes. The current configuration captures every transaction for full performance visibility.


</ParamField>

<ParamField body="tracePropagationTargets" param-type="array" required="true" show-location="false">

**Purpose:** Define which request targets receive Sentry trace headers for distributed tracing.

**Type:** array of strings and regex patterns.

**Explicit value:** same-origin `/api/` requests and the URL set in `VITE_API_URL` when provided.

**Required:** Yes. This stitches browser traces to API service traces so a single user action appears as one unified trace across services.


</ParamField>

<ParamField body="sendDefaultPii" param-type="boolean" required="true" show-location="false">

**Purpose:** Include personally identifiable information in Sentry events.

**Type:** boolean.

**Explicit value:** `true`.

**Required:** Yes. Enabled on both web and API services.
...
Setting `tracesSampleRate` to `1.0` captures 100% of transactions, which provides complete performance visibility but may increase Sentry transaction volume. If Sentry plan quotas are a concern, lower the API service rate via the `SENTRY_TRACES_SAMPLE_RATE` environment variable without changing source code.
