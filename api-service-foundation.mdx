...
description: >-
  How the NestJS API service boots, wires modules, enables CORS, exposes health
  checks, and builds for local and containerized execution.
---

## Overview

The API service is a minimal NestJS foundation app in `apps/api`. It boots a single Nest application, enables CORS for one local frontend origin, and listens on a fixed port. The surrounding build and delivery setup already includes Bun scripts, a Dockerfile, and a GitHub Actions workflow. Bounty funding has now added a real application slice with authentication guards, validated DTOs, Supabase-backed persistence, and domain-specific modules and services, even though the broader service still remains a foundation outside that slice.

<Callout kind="info">

The current implementation is still intentionally small outside the bounty funding slice. Alongside `GET /health`, the inspected source now includes bounty funding endpoints that expand the module graph with bounty controllers, services, and guards.
...
Because the bootstrap file is still minimal, startup remains minimal at the application entrypoint. Bounty funding endpoints now use authentication guards and validation pipes, but the inspected `main.ts` still does not add global pipes, interceptors, guards, filters, or prefixes.
...
`AppModule` is the full application root, and it now wires more than the health controller alone. Bounty funding has introduced additional controllers, a service layer, and providers, while the health portion of the app remains simple.
...
AppModule
├── HealthController
└── Bounty funding controllers, services, and guards
```
...
The module graph is still compact, but it is no longer limited to the health controller:

- `AppModule` still includes `HealthController`
- bounty funding adds additional controllers behind the same application root
- bounty funding adds providers and a service layer
- the health endpoint remains a direct, simple controller route

That means the inspected service now includes one real domain slice with controller, service, auth, validation, and persistence concerns, even though the overall app tree is still much smaller than a fully built backend.

### What is not wired yet

Several common Nest application concerns are still absent or only partially implemented in the inspected files:

- Supabase JWT auth guards are now used on bounty endpoints, but auth is not a broad application-wide layer
- bounty payment verification now uses a validated DTO, but DTO coverage is still narrow
- bounty funding now uses Supabase-backed persistence with SQL migrations, but persistence is not yet a general platform layer
- bounty funding has introduced domain-specific modules and services, but only for that slice
- `ConfigModule` or other environment-driven configuration is still largely absent, although Razorpay environment variables are now read for payment configuration
...
`GET /health` is still the simplest implemented endpoint, but it is no longer the only one. Bounty funding endpoints have been added alongside it, while the health route still returns a plain inline object and does not use a DTO, schema, or health-check integration package.
...
- DTOs limited to bounty payment verification
- auth limited to bounty funding endpoints
- persistence limited to bounty funding via Supabase
- domain modules limited to bounty funding
- no broad environment-based configuration layer, although Razorpay environment variables are now read for payment setup
...
The implementation is now enough to boot a Nest service, answer a health check, serve a real bounty funding domain slice with auth, persistence, and validation, build with TypeScript, package into a container, and publish through CI. The broader service still needs the same infrastructure improvements, especially around fixed port configuration, Docker port exposure, and the likely built-entry path mismatch between `main` and `start`.
