...
The inspected source code on the main branch does not consume repository-defined application environment variables. The bounty funding feature, merged on a feature branch, introduces configuration for Razorpay payment processing and Supabase persistence. Specific environment variable names will be documented here once the code is merged to main.

<Callout kind="success">

No `.env` files were found in the main branch repository tree, and no application code reads of `process.env`, `import.meta.env`, `os.getenv`, `getenv`, or `dotenv` were found on main. The bounty funding feature on the feature branch is expected to introduce environment-based configuration for Razorpay and Supabase.
...
<ParamField body="bounty-funding-config" param-type="environment variables" required="true" show-location="false">

**Purpose:** Configure Razorpay payment processing and Supabase persistence for bounty funding.

**Type:** environment variables (exact names pending main-branch merge).

**Explicit value:** not yet documented — the bounty funding code is on a feature branch.

**Required:** Yes for bounty funding endpoints to function. Razorpay key ID and key secret are needed for order creation and signature verification. Supabase connection configuration is needed for `bounty_orders` and `bounties` table access.
...
The bounty funding feature uses a runtime check (`razorpay.isEnabled`) to determine whether the pay-to-post flow is active. When Razorpay is not configured, bounties are created with status `open` and no payment is required. This acts as an implicit feature flag for the pay-to-post flow.
