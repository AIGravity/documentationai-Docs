...
The API service is a minimal NestJS foundation app in `apps/api`. It boots a single Nest application, registers one `HealthController`, enables CORS for one local frontend origin, and listens on a fixed port. The surrounding build and delivery setup already includes Bun scripts, a Dockerfile, and a GitHub Actions workflow, and the service now includes bounty funding with JWT authentication via `SupabaseAuthGuard`, a class-validator DTO (`VerifyBountyOrderDto`), Supabase-backed persistence, and domain modules for bounties and payments. OpenAPI/Swagger setup is not yet configured.

<Callout kind="info">

The health endpoint remains the simplest route, but the module graph now also includes bounty controllers, a `BountyFundingService`, a `SupabaseService`, and a `SupabaseAuthGuard`.
...
`AppModule` is the application root. In addition to `HealthController`, the module graph now includes bounty and payment controllers, a `BountyFundingService`, a `SupabaseService` for persistence, and a `SupabaseAuthGuard` for JWT authentication.
...
- `AppModule` declares `HealthController` plus bounty and payment controllers
- `AppModule` imports bounty and payment feature modules
- `AppModule` provides `SupabaseService` and `BountyFundingService`
- `AppModule` exports nothing

The bounty funding flow uses a service layer (`BountyFundingService`), a persistence layer (`SupabaseService`), and a DTO layer (`VerifyBountyOrderDto`). The health endpoint remains a standalone controller without those layers.
...
The bounty funding module demonstrates a controller-to-service-to-persistence pattern. The health endpoint remains a minimal standalone controller.
...
The health endpoint is the simplest route in the API. It returns a plain inline object and does not use a DTO, schema, or health-check integration package.
...
The bounty funding feature has added auth, DTOs, persistence, and domain modules, but several foundation-level concerns remain:
...
The implementation now includes bounty funding with auth, persistence, and payment integration. The remaining foundation-level work includes resolving the fixed port configuration, Docker port exposure, the likely built-entry path mismatch between `main` and `start`, and adding environment-based configuration and OpenAPI setup.