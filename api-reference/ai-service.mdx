...
- **`404 Not Found`** — Returned when you request a path that is not defined. The FastAPI framework returns `{"detail": "Not Found"}`.
- **`405 Method Not Allowed`** — Returned when you use an HTTP method that is not registered for an existing path (e.g., `POST /health`). FastAPI returns `{"detail": "Method Not Allowed"}`.
