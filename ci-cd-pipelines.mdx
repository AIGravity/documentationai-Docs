...
A separate deployment workflow provides on-demand deployment to AWS ECS for any of the three services. This workflow is manually triggered and supports multiple environments, building and pushing images to Amazon ECR and rolling out new task definitions on an ECS cluster.
...
- **`.github/workflows/deploy-new-account.yml`** — `New-Account Deployment`

The first three define the same job chain:
...
## New-account deployment workflow

The `New-Account Deployment` workflow runs only when you start it manually with `workflow_dispatch`. Pushes and pull requests do not trigger this workflow.

### Inputs select the deployment target

The workflow requires an environment and service. You can optionally choose the Git ref, image tag, and Redis endpoint.
  ...
  workflow_dispatch:
    inputs:
      environment:
        required: true
        type: choice
        options: [nonprod, prod, dev]
      service:
        required: true
        type: choice
        options: [api, web, ai-service]
      ref:
        required: false
      image_tag:
        required: false
...
When `ref` is omitted, the workflow selects the `nonprod` branch for `nonprod` and `prod`, and the `dev` branch for `dev`. When `image_tag` is omitted, it uses the short commit SHA. `redis_host` overrides the Redis endpoint for `ai-service` deployments.

### Environment mapping controls branches and secrets

The workflow maps each input environment to a branch and GitHub Actions environment:

| Input environment | Branch | GitHub environment |
| --- | --- | --- |
| `nonprod` | `nonprod` | `nonprod` |
| `prod` | `nonprod` | `nonprod` |
| `dev` | `dev` | `development` |

The `prod` option therefore uses the `nonprod` branch and `nonprod` environment for its deployment secrets.

### Build and push service images

The workflow builds the selected service from its existing Dockerfile, then pushes the image to Amazon ECR with the selected tag.

```yml
docker build \\
  --file apps/${SERVICE}/Dockerfile \\
  --tag ${ECR_REGISTRY}/gravity-${SERVICE}:${IMAGE_TAG} \\
  .
docker push ${ECR_REGISTRY}/gravity-${SERVICE}:${IMAGE_TAG}
```

For web builds, the workflow copies `tsconfig.base.json` into the Docker build context and passes the `VITE_*` build arguments. It also passes `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, and `SENTRY_PROJECT` as Sentry build arguments.

### Update the ECS service

After pushing the image, the workflow registers a new ECS task definition from the template for the selected environment. It then forces the ECS service to start a deployment with the new task definition.

```yml
aws ecs update-service \\
  --cluster gravity-${ENVIRONMENT} \\
  --service gravity-${SERVICE} \\
  --task-definition ${TASK_DEFINITION} \\
  --force-new-deployment
```

The deployment role supplies AWS credentials, and the cluster and service names follow the `gravity-{environment}` and `gravity-{service}` conventions.

### Concurrency preserves in-progress deployments

Runs use the concurrency group `deploy-new-${environment}-${service}`. A new run for the same environment and service does not cancel an in-progress deployment.

<Callout kind="info">

The web Dockerfile does not currently declare Sentry `ARG` instructions. The workflow passes the Sentry build arguments, but the Dockerfile does not consume them.
...
- Deploy jobs in the three service workflows do not execute a real deployment command. Use the separate [new-account deployment workflow](#new-account-deployment-workflow) for on-demand ECS deployment.
