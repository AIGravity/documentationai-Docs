...
The API service is a minimal NestJS foundation app in `apps/api`. It boots a single Nest application, registers one `HealthController`, enables CORS for one local frontend origin, and listens on a fixed port. The surrounding build and delivery setup already includes Bun scripts, a Dockerfile, and a GitHub Actions workflow, and the service includes a chat module with DTOs and a service layer that processes chat messages, but does not yet include auth, persistence, OpenAPI, or environment-based configuration.

<Callout kind="info">

The service exposes two endpoints: `GET /health` for liveness checks and `POST /api/chat/message` for chat interactions. The chat module includes DTOs for request validation and a service layer that orchestrates calls to the downstream AI service.
...
`AppModule` is the application root. The module graph includes the health controller for liveness checks and the chat module for chat interactions. The chat module provides a service that handles message processing, AI service communication, and response transformation.
...
- `AppModule` declares the health controller for the `/health` endpoint
- The chat module handles `POST /api/chat/message` with its own controller, service, and DTOs
- The chat service orchestrates calls to the downstream AI service and enriches results with published workflow metadata

That means the service now includes a clear separation between HTTP handling and chat-processing logic. The health route remains controller-only, while chat interactions flow through DTOs and a dedicated service layer.
...
The module graph includes a service layer for chat processing that abstracts AI service communication and response transformation away from the controller.
...
The health endpoint is one of the service's implemented routes. It returns a plain inline object and does not use a DTO, schema, or health-check integration package. The service also exposes `POST /api/chat/message` for chat interactions. See the NestJS API reference for full endpoint documentation.
