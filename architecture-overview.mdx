...
`apps/web` is the user-facing frontend. It uses Remix with Vite and React, and has begun introducing product-specific UI including a builder dashboard with sidebar navigation and a leaderboard page.
...
The index and home routes render the same welcome component from `apps/web/app/welcome/welcome.tsx`. A separate `/leaderboard` route renders the builder leaderboard page with incentive cards. There are no route loaders, no route actions, no session handling, no data fetching helpers, and no API client layer in the current frontend tree.
...
- `/leaderboard` renders the builder leaderboard page with incentive cards

The index and home route modules render the same welcome component. There are no loaders, actions, or backend requests in those routes.
...
The practical result is a stateless system. The frontend renders a starter welcome screen alongside builder-specific UI including the leaderboard, the API returns a hardcoded health response, and the AI service returns a hardcoded health response.
