...
The current frontend is a thin Remix application shell with two implemented routes, a shared starter-style welcome screen, global Tailwind-based styling, and basic packaging for local development and containers. It establishes the document structure, asset pipeline, and runtime boundaries for the web app. The shell now also hosts product-specific routes — including bounty browsing, agent customization, and submission review — backed by an API client layer for backend communication.

## App purpose and current scope

The frontend has grown beyond its initial scaffold. While the root layout and styling pipeline remain from the starter template, the application now includes product-specific routes for bounty browsing, agent customization, and submission review, with an API client for backend communication.

<Callout kind="info">

This page reflects the implementation in `apps/web`. The shell includes routing, layout, styling, error handling, local scripts, and Docker packaging. It now also includes app-specific screens for bounty browsing and agent submission, API calls through a client layer, and workflow UI.
...
The shell's two starter routes render the same shared component. Additional product-specific routes for bounty browsing and agent customization have been added on top of the shell.
...
Because both starter routes render the same component, there is no functional distinction between `/` and `/home`. Product-specific routes have been added separately on top of the shell.
...
The visible UI for the two starter routes lives in `apps/web/app/welcome/welcome.tsx`. It is a placeholder screen intended to confirm that the shell is running. Product-specific routes have their own UI components.
...
The frontend shell does not yet include authentication or session handling. It provides routing, layout, styling, error handling, packaging, and now also product-specific routes for bounty browsing and agent submission with API client integration.


</Callout>

Authentication, session handling, and shared app chrome are areas that still need to be built out on top of the existing shell.
