...
description: >-
  How the repository’s GitHub Actions workflows build, check, version, and
  publish the web, API, and AI service artifacts.
---
...
The AI service workflow sets `apps/ai-service` as the working directory, installs the `uv` tool, provisions Python with `uv`, syncs dependencies, then runs Ruff format and lint checks.
...
| AI service | `.github/workflows/ai-service.yml` | Install `uv`, install Python, sync dependencies, Ruff format check, Ruff lint | `apps/ai-service/**`, workflow file | Uses `uv` and Python tooling instead of pnpm-based Node workflow |

### Representative build sections

<CodeGroup tabs="Web,API,AI service,AI service Python toolchain">
...
```python
# apps/ai-service build job command order
uv_commands = [
    "uv python install",
    "uv sync",
