...
The following environment variables were removed from the API service in the latest platform update. If your local or deployment environment still references them, remove all references to avoid confusion.

<ParamField body="INNGEST_DEV" param-type="string" required="false" show-location="false">

**Purpose:** Previously configured Inngest development mode for the durable workflow engine.

**Status:** **Removed.** Inngest has been fully decommissioned. The `/api/inngest` handler, all durable workflow functions, and the `inngest` and `inngest-cli` dependencies were removed.


</ParamField>

<ParamField body="INNGEST_EVENT_KEY" param-type="string" required="false" show-location="false">

**Purpose:** Previously held the Inngest event key for dispatching events to durable functions.

**Status:** **Removed.** No longer referenced by any source file.


</ParamField>

<ParamField body="INNGEST_SIGNING_KEY" param-type="string" required="false" show-location="false">

**Purpose:** Previously held the Inngest signing key for verifying webhook payloads.

**Status:** **Removed.** No replacement is needed since Inngest is no longer used.


</ParamField>

<ParamField body="INNGEST_ENV" param-type="string" required="false" show-location="false">

**Purpose:** Previously selected the Inngest environment (e.g., `dev` or `production`).
...
<ParamField body="AP_USER_PASSWORD_SECRET" param-type="string" required="false" show-location="false">

**Purpose:** Previously used for HMAC-based user password generation when minting Activepieces sessions.

**Status:** **Removed.** Direct JWT minting via `AP_JWT_SECRET` now replaces the password-based flow.
...
Remove all five Inngest variables and `AP_USER_PASSWORD_SECRET` from your `.env` files, Docker compose configurations, and deployment secrets. They are no longer read by the API service and leaving them in place has no effect but may cause confusion during setup.
