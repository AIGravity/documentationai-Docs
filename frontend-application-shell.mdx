...
The application shell is implemented with Remix v2. The current starter screen still exposes React Router placeholder assets in the shared `Welcome` component, including `React Router` image alt text and a React Router documentation resource link. Treat those visible labels as starter UI placeholders, not as evidence that the app is built on React Router.
