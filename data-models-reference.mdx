...
What exists today includes two inline response shapes returned by health endpoints and the agent search response shape returned by the AI service's recommendation engine. These shapes are API responses, not persisted entities, and they do not declare relationships, validation rules, or storage mappings.
...
The repository is developing its first application-level response shapes through the AI service's agent recommendation engine, though it still lacks named domain models and persistence entities.
...
- **Agent search response** — inline response shape with `matched_platforms`, `matched_agent_types`, and `matched_tools` arrays returned by the AI service's recommendation engine
...
- The FastAPI service exposes a health endpoint with an inline object response and returns agent search results through the recommendation engine.
...
## Agent search response shape

The AI service's recommendation engine returns agent search results as inline response shapes. Each result includes arrays showing which of the user's selected preferences matched the agent's capabilities.

### Classification

- **Kind** — Inline response shape
- **Named model** — None found in the analyzed code
- **Persistence-backed** — No (agent data is stored in Typesense, not in a local database)
- **Relationships** — None found in the analyzed code
- **Validation rules** — None found in the analyzed code

### Response fields

<ResponseField name="matched_platforms" field-type="array" required="true">

Platforms from the user's selection that match the agent's supported platforms. Always present, even when empty.


</ResponseField>

<ResponseField name="matched_agent_types" field-type="array" required="true">

Agent types from the user's selection that match the agent's types. Always present, even when empty.


</ResponseField>

<ResponseField name="matched_tools" field-type="array" required="true">

Tools from the user's selection that match the agent's supported tools. Always present, even when empty, so consumers can inspect which tools contributed to a ranking boost.
...
These response shapes are constructed inline in the recommendation engine's service layer. No Pydantic response model is defined for the agent search results. The `matched_tools` array was added to provide transparency into which tool selections influenced the ranking.
