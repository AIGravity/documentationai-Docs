...
title: Integrations
description: >-
  Connect documentation content and tooling workflows to your development
  process.
---
...
Confirm the content change, metadata, and links before merging. The review should show the exact files that changed.
...
If your docs rely on custom MDX components, keep the build check close to the editor workflow so syntax errors surface before merging.
...
CI verification runs the docs build after installation and before deployment.
...
Because exact event names are not sourced here, define your own event contract around the actions you already need, such as a content update, page publication, or build completion. Keep the payload stable and small so downstream automation stays predictable.
...
<Card title="Docs workflow" href="/help-center/guides/add-project-docs" icon="book-open" horizontal={true}>

See how documentation pages fit into the broader documentation workflow.

</Card>

<Card title="Repository automation scripts" href="/repository-automation-scripts" icon="settings" horizontal={true}>
