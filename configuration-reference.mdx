...
This repository currently relies far more on static configuration than on environment-driven configuration. The AI service reads environment variables through `os.getenv()` for LLM configuration and search thresholds, and ships a `.env.example` file documenting those variables. The web and API services still rely on hardcoded configuration. No app-level feature flag system is implemented.
...
The AI service consumes environment variables for its dual-LLM interaction pipeline. These variables control LLM temperature, token limits, and the vector search distance threshold.

<Callout kind="info">

The AI service reads the following environment variables through `os.getenv()` in its config module. A `.env.example` file documents the defaults. The web and API services do not currently read environment variables in application code.
...
<ParamField body="GUIDE_TEMP" param-type="float" required="false" show-location="false">

**Purpose:** Set the temperature for the guide LLM (LLM1) that produces deterministic JSON mode and spec output.

**Type:** float.

**Explicit value:** `0.0`.

**Required:** No. Defaults to `0.0` when unset.


</ParamField>

<ParamField body="RESPONDER_TEMP" param-type="float" required="false" show-location="false">

**Purpose:** Set the temperature for the responder LLM (LLM2) that writes user-facing replies.

**Type:** float.

**Explicit value:** `0.3`.

**Required:** No. Defaults to `0.3` when unset.


</ParamField>

<ParamField body="GUIDE_MAX_TOKENS" param-type="integer" required="false" show-location="false">

**Purpose:** Set the maximum completion tokens for the guide LLM.

**Type:** integer.

**Explicit value:** `700`.

**Required:** No. Defaults to `700` when unset.
...
**Purpose:** Set the vector distance threshold for agent search matching. Lower values produce stricter matching with fewer but higher-quality results. The default changed from `0.30` to `0.22` in the dual-LLM update.

**Type:** float.

**Explicit value:** `0.22`.

**Required:** No. Defaults to `0.22` when unset.


</ParamField>

<CodeGroup tabs=".env.example">

```bash
# AI service environment variables (.env.example)
SEARCH_VECTOR_DIST_THRESHOLD=0.22
GUIDE_TEMP=0.0
RESPONDER_TEMP=0.3
GUIDE_MAX_TOKENS=700
```
...
The AI service also reads environment variables for the dual-LLM interaction pipeline. See the Environment variables section above for `GUIDE_TEMP`, `RESPONDER_TEMP`, `GUIDE_MAX_TOKENS`, and `SEARCH_VECTOR_DIST_THRESHOLD`.
