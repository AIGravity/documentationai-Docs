...
A new global CSS rule also forces `color-scheme: light` on Razorpay checkout iframes. That change fixes a rendering issue where the app's dark color scheme caused the cross-origin Razorpay checkout iframe to display with an opaque white background instead of a transparent overlay. The rule targets `.razorpay-checkout-frame`, `.razorpay-container iframe`, and any iframe whose `src` contains `razorpay`.
...
The frontend now includes a dashboard route with bounty creation and a Razorpay payment flow, which introduces authentication, API integration, and business workflow UI alongside the starter routes described on this page. The starter routes themselves still remain simple scaffolds for routing, layout, styling, error handling, and packaging.


</Callout>

That change is important when planning the next layer of frontend work. The dashboard route and bounty payment flow are now in place, while broader frontend architecture improvements such as additional shared app chrome, expanded route structure, and deeper API-facing components are still needed.
