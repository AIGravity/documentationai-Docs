...
The current frontend is a Remix application that includes a builder dashboard with sidebar navigation, a leaderboard page with builder incentive cards, and a starter-style welcome screen. It uses global Tailwind-based styling and basic packaging for local development and containers. The app now contains product-specific UI for the builder experience, though it does not yet include authentication or backend-driven data flows.

## App purpose and current scope

Today, the frontend extends beyond the initial scaffold to include a builder dashboard with sidebar navigation and a leaderboard page displaying builder incentive cards. The welcome screen remains as a starter-style landing page, but the app now has product-specific screens and workflow UI alongside the foundational shell.

<Callout kind="info">

This page reflects the current implementation in `apps/web`. The shell includes routing, layout, styling, error handling, local scripts, and Docker packaging. It now includes app-specific screens such as the builder dashboard and leaderboard, but does not yet include API calls, session handling, or backend data integration.
...
The index and home routes render the same shared welcome component. A `/leaderboard` route provides the builder leaderboard page with incentive cards.
...
The `/leaderboard` route renders the builder leaderboard page. In its current pre-launch state, the page displays builder incentive cards highlighting revenue share, per-agent earnings, and internship opportunities. A leaderboard navigation button in the builder sidebar provides direct access to this route from any builder page.

While `/` and `/home` remain functionally identical starter routes, the `/leaderboard` route introduces the first product-specific screen in the frontend.
...
At this stage, the welcome screen is not connected to any application behavior. The welcome screen itself has:
...
The builder dashboard and leaderboard page provide domain-specific navigation and workflow UI elsewhere in the app. The two external links in the resources list open in a new tab, and the welcome screen remains a starter-style landing page rather than an application homepage.
...
The frontend shell does not yet include authentication or backend data integration. It provides routing, layout, styling, error handling, packaging, and product-specific UI including the builder dashboard and leaderboard.
...
That constraint is important when planning the next layer of frontend work. Loaders, actions, API-facing components, and authentication still need to be introduced on top of this shell.
