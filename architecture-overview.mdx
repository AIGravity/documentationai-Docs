...
The practical architecture today is a deployment-ready foundation with an emerging application layer in the AI service. Each service starts, builds, and ships independently. The AI service hosts an agent recommendation engine that integrates with Gemini and Typesense, while the web and API services remain at the foundation stage. There are no shared packages, no authentication system, and no implemented cross-service request flow beyond the API allowing browser requests from `http://localhost:3000`.
...
`apps/ai-service` is the Python service that hosts the agent recommendation engine and a health check endpoint. The service integrates with Gemini for query embeddings and Typesense for vector search to power agent discovery and ranking.

The entry point is `apps/ai-service/main.py`. That file creates a `FastAPI` app and defines `GET /health`, which returns a static status object. The agent recommendation engine is implemented as a separate service layer that handles agent search and ranking, including a soft-blend formula that considers semantic similarity, platform overlap, agent type matching, and tool selection.

The Python project configuration lives in `apps/ai-service/pyproject.toml`. Dependencies include `fastapi`, `pydantic`, and `uvicorn`, with `ruff` and `pytest` declared for development. Additional packages for Gemini and Typesense integrations support the recommendation engine.

<Callout kind="info">

The agent recommendation engine integrates with Gemini for query embeddings and Typesense for vector search. Ranking applies a soft-blend formula that combines semantic distance with platform, agent type, and tool-matching signals.
...
- Agent search and recommendation — searches for and ranks agents based on a user's query and selected platforms, agent types, and tools

The recommendation engine is implemented in dedicated service modules separate from the health check route.
...
Inside those boundaries, the internal module graph is still mostly absent. The Nest app does not yet follow a controller-to-service-to-repository pattern. The FastAPI app separates business logic into dedicated service modules for agent search and recommendation ranking. The frontend does not yet separate route modules from API clients, domain packages, or shared contracts.

That means the current architecture is macro-separated, with the AI service beginning to develop internal layering through its recommendation service modules. The web and API services have not yet developed comparable internal structure.
...
The practical result is a stateless system for the web and API services. The frontend renders starter UI and the API returns a hardcoded health response. The AI service uses Typesense as an external vector search index for agent retrieval, but no traditional database or local persistence layer is implemented within the repository.
...
- Gemini model provider API for query embeddings in the AI service
- Typesense vector search for agent retrieval in the AI service
...
The Python service declares `pytest` and includes unit tests for the agent search and ranking logic. The CI workflows reference verification steps, though the pytest step remains commented out in the workflow configuration. The web and API services do not yet include test suites.

<Callout kind="info">

The strongest part of the current architecture is operational structure: app boundaries, runtime separation, containerization, and release automation. The AI service has begun building application depth with its agent recommendation engine, but the web and API services still lack domain modules, persistence, auth, and implemented service-to-service flows.
...
What it has not answered yet are some application-level questions: how contracts are shared, how users authenticate, and how the API and AI service collaborate. The AI service now includes application logic for agent recommendation, but the web and API services remain at the foundation stage. The repo should be evaluated as a well-structured platform with an emerging application layer in the AI service.
