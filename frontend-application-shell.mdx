...
The current frontend is a Remix application shell with three implemented routes: two starter-style welcome screens and an agent customization workflow. The shell establishes the document structure, asset pipeline, and runtime boundaries for the web app, and includes a product-specific agent customization screen with AI-assisted content generation and backend API integration.

## App purpose and current scope

Today, the frontend serves as a scaffold with one product-specific route. The two welcome routes render the same shared `Welcome` component, while the agent customization route provides a multi-step workflow with AI-assisted content generation and backend API integration.

<Callout kind="info">

This page reflects the current implementation in `apps/web`. The shell includes routing, layout, styling, error handling, local scripts, and Docker packaging. The agent customization route adds app-specific screens, API calls, and workflow UI on top of the base shell.
...
Three routes are currently implemented: two render the same shared welcome component, and one provides a product-specific agent customization workflow.
...
The `/customize-agent` route is defined in `apps/web/app/routes/customize-agent.tsx`. It is the first product-specific screen in the frontend and provides a multi-step agent customization workflow.

The first step collects three agent detail fields: title, description, and problem solved. When a builder uploads a workflow and lands on this step, the form automatically generates AI-written content for all three fields in parallel, using the uploaded workflow's metadata as input. Each field displays a "Writing with AI…" indicator while generation is in progress. Once the AI results arrive, the fields populate automatically. If the builder starts typing in a field before the AI result for that field arrives, the builder's input is preserved and the AI result is discarded.

The automatic generation only runs for new agent creation sessions. It is skipped when editing an existing agent, resuming a saved session, or re-entering from a bounty flow. In those cases, the builder's previously saved content is used instead.

The route calls the API service's preview-description endpoint to generate the AI content, making it the frontend's first integration with a backend API.

Because both welcome routes render the same component, there is no functional distinction between `/` and `/home`. The `/customize-agent` route is the first product-specific screen and the first route with backend API integration.
...
The frontend shell does not yet include authentication or shared app chrome such as navigation bars and global providers. The two welcome routes remain starter-style placeholder screens. The `/customize-agent` route is the only route with business workflow UI and backend API integration; all other routes are scaffold-level only.


</Callout>

Those constraints are important when planning the next layer of frontend work. Authentication, shared app chrome, and additional product-specific routes still need to be introduced on top of the existing shell and customize-agent workflow.
