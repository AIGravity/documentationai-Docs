...
description: >-
  How the Remix frontend is scaffolded today, including root layout, routes,
  styling, error handling, and packaging.
---

## Overview

The current frontend is a Remix application shell with a starter-style welcome screen and a competitive leaderboard page, global Tailwind-based styling, and basic packaging for local development and containers. It establishes the document structure, asset pipeline, and runtime boundaries for the web app, and now includes a leaderboard route with auth-aware navigation and data-driven competitive rankings.

## App purpose and current scope

The frontend serves as a scaffold for most routes, with a competitive leaderboard page as the first product-specific screen. The index and home routes render the same shared `Welcome` component as placeholders, while the leaderboard route provides a full competitive rankings experience.

<Callout kind="info">

This page reflects the current implementation in `apps/web`. The shell includes routing, layout, styling, error handling, local scripts, and Docker packaging. The leaderboard route adds the first app-specific screen with auth-aware navigation and data-driven rankings, while the index and home routes remain scaffold-level placeholders.
...
- `apps/web/app/routes/leaderboard.tsx`
...
Three routes are currently implemented. The index and home routes render the same shared welcome component, while the leaderboard route provides a standalone competitive rankings page.
...
Because the index and home routes render the same component, there is no functional distinction between `/` and `/home`.

### `/leaderboard`

The leaderboard route is defined in `apps/web/app/routes/leaderboard.tsx`. It is the first product-specific route on the frontend and displays competitive rankings across three phases:

- **Locked** — Before the leaderboard opens, users see a full-screen teaser with prize highlights (LinkedIn recommendation, recurring earnings, job offer) and a countdown clock to the opening time.
- **Live** — Once open, the leaderboard shows a top-three podium with prize amounts and a scrollable rankings table starting at rank 4. Scores are displayed to two decimal places.
- **Results** — After the competition ends, the final standings are shown in a frozen view.

The navigation bar includes a call-to-action button that routes logged-in users to their dashboard and new users to the onboarding flow. A footer at the bottom of the page links to the project website and social profile.
...
At this stage, the welcome screen used by the index and home routes is not connected to any application behavior. The current implementation has:

- no API integration on the welcome screen
- no backend data fetches on the welcome screen
- no auth or session UI on the welcome screen
- no domain-specific navigation on the welcome screen
- no business workflow screens on the welcome screen

The two external links in the resources list open in a new tab, and the welcome screen remains a starter-style landing page rather than an application homepage. The leaderboard route, by contrast, provides auth-aware navigation and data-driven competitive rankings.
...
The frontend shell provides routing, layout, styling, error handling, and packaging. The leaderboard route adds auth-aware navigation and data-driven rankings, but most other routes remain scaffold-level placeholders without product-specific UI.


</Callout>

That constraint is important when planning the next layer of frontend work. The leaderboard route is the first product-specific addition; additional pages, loaders, actions, shared app chrome, and API-facing components still need to be introduced on top of this shell.
