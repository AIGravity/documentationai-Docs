...
The web app also includes Sentry entry points for browser-side and server-side error monitoring and performance tracing. Sentry activates only in production environments and propagates trace headers to the API so that browser and backend spans appear as one unified trace.
...
- Sentry error monitoring and performance tracing across the web and API services
...
Sentry is configured in the web app's browser and server entry points and in the API service's bootstrap. It activates only in production environments and captures 100% of transactions for full performance visibility. Trace headers propagate from the browser through to the API so that a single user action appears as one unified trace across services.
