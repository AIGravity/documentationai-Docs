...
The web app includes Sentry entry points for error monitoring and performance tracing in production environments.

### Browser-side Sentry

The browser entry point initializes Sentry only when a DSN is present and the environment is set to `production`. It captures 100% of transactions for full performance visibility and propagates trace headers to same-origin `/api/` requests and to the URL configured in `VITE_API_URL`, so browser and backend spans appear as one unified trace across services.

A user feedback widget is also integrated, allowing users to submit bug reports, feedback, and feature requests directly from the browser.

### Server-side Sentry

The server entry point initializes Sentry under the same production gate and wraps the Remix error handler so that server-side runtime errors are automatically captured and reported to Sentry.

<Callout kind="info">

Sentry is completely disabled in non-production environments. No configuration is required for local development.


</Callout>

See the [Configuration Reference](/configuration-reference#sentry-tracing-configuration) for the full list of Sentry environment variables and tracing settings.
