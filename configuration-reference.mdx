...
This repository currently relies far more on static configuration than on environment-driven configuration. The inspected implementation does not define any `.env` files and does not implement an app-level feature flag system. The AI service reads one runtime environment variable, `SEARCH_VECTOR_DIST_THRESHOLD`, for vector search distance threshold configuration.
...
- **GitHub Actions workflows** define service names, image names, branch filters, concurrency behavior, setup versions, release gating logic, and deploy-time environment variables for the AI service.
...
The AI service reads one runtime environment variable for vector search configuration. No other application code in the repository consumes environment variables.

<Callout kind="info">

No `.env` files were found in the repository tree. The AI service reads `SEARCH_VECTOR_DIST_THRESHOLD` via `os.getenv` with a hardcoded fallback of `0.30`. No `process.env`, `import.meta.env`, `getenv`, or `dotenv` usage was found in the API or web service code.
...
<ParamField body="SEARCH_VECTOR_DIST_THRESHOLD" param-type="float" required="false" show-location="false">

**Purpose:** Set the maximum cosine distance for vector search results to survive filtering in the AI service agent search. Lower values produce fewer but more semantically relevant results.

**Type:** float.

**Default:** `0.30`. The deployment pipeline injects this value from the GitHub repository variable `AI_SEARCH_VECTOR_DIST_THRESHOLD`.

**Required:** No. If the environment variable is not set, the AI service falls back to a hardcoded default of `0.30`.
...
**Purpose:** Set the vector distance threshold for the AI service at container runtime via the ECS task definition.

**Type:** float.

**Explicit value:** Injected at deploy time from the GitHub repository variable `AI_SEARCH_VECTOR_DIST_THRESHOLD`. The ECS task definition includes this variable in the container environment.

**Required:** No. If absent, the AI service application code falls back to a hardcoded default of `0.30`.
...
**Purpose:** Set the vector distance threshold for the AI service at deploy time.

**Type:** float.

**Explicit value:** Sourced from the GitHub repository variable `AI_SEARCH_VECTOR_DIST_THRESHOLD` via `${{ vars.AI_SEARCH_VECTOR_DIST_THRESHOLD }}`. Set in both staging and production deploy jobs of the AI service workflow.

**Required:** No. If the repository variable is not set, the container environment variable is empty and the AI service falls back to a hardcoded default of `0.30`.
...
<CodeGroup tabs="AI service Dockerfile,Workflow env,AI service deploy env">
...
```yaml
# AI service deploy job environment (staging and production)
SEARCH_VECTOR_DIST_THRESHOLD: ${{ vars.AI_SEARCH_VECTOR_DIST_THRESHOLD }}
```
...
<ParamField body="vector-dist-threshold" param-type="float" required="false" show-location="false">

**Purpose:** Set the maximum cosine distance for vector search hits to survive filtering in the AI service agent search. Applies to both the Typesense vector query distance threshold and client-side candidate ranking.

**Type:** float.

**Default:** `0.30` (environment-overridable via `SEARCH_VECTOR_DIST_THRESHOLD`).

**Required:** No. If the environment variable is not set, the application uses the hardcoded default. Lower values produce fewer but more semantically relevant results.
