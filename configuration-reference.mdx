...
description: Reference for the repository's current environment variables, config files, hardcoded runtime settings, database-stored pricing configuration, and workflow gates across the web, API, and AI services.
---
...
## Pricing configuration (database-stored)

The billing system reads runtime-configurable parameters from the `pricing_config` database table. These entries control the creator split behavior for run charges and chat charges. No database migration is required to change them — the application reads them at charge time and falls back to hardcoded defaults when the rows are absent.

<ParamField body="platform_share_pct" param-type="integer (value_int)" required="false" show-location="false">

**Purpose:** Set the platform's share of the creator split as a percentage. The creator earns 100 minus this value.

**Type:** integer stored in the `value_int` column.

**Explicit value:** No checked-in default in the database. The application falls back to `80` when the row is absent.

**Required:** No. When absent, the default of `80` is used.
...
<ParamField body="creator_share_basis" param-type="string (value_text)" required="false" show-location="false">

**Purpose:** Determine how the creator's share is calculated from each run charge.

**Type:** string stored in the `value_text` column.

**Explicit value:** No checked-in default in the database. The application falls back to `gross` when the row is absent.

**Required:** No. When absent, the default of `gross` is used.

**Accepted values:**

- `gross` — The creator's share is a percentage of the full run charge.
- `margin` — The creator's share is a percentage of the charge minus the measured LLM cost.
...
When `creator_share_basis` is set to `gross`, the charge RPC receives `rawCost = 0`, so the creator earns a percentage of the full charge amount. When set to `margin`, the RPC receives the measured LLM cost, and the creator earns a percentage of the difference between the charge and the cost. The default behavior is `gross`.
...
See the Data Models Reference for the full `pricing_config` table schema and the `creator_earnings_ledger` table that records per-run creator earnings.
