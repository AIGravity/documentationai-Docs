...
description: Reference for the repository's environment variables, config files, hardcoded runtime settings, and workflow gates across the web, API, and AI services, including search ranking weights and currency configuration.
---

## Overview

This repository currently relies far more on static configuration than on environment-driven configuration. The repository uses a mix of static configuration and environment-driven settings. The AI service reads environment variables for search ranking behavior, and the web app defines configuration for supported currencies.
...
The AI service reads environment variables to control agent search ranking behavior. The NestJS API and web app continue to rely primarily on static configuration.

<ParamField body="SEARCH_TOOL_WEIGHT" param-type="number" required="false" show-location="false">

**Purpose:** Control the weight of tool coverage in agent search ranking. Higher values prioritize agents whose required tools are already connected by the user.

**Type:** number (float).

**Default value:** 0.20.

**Required:** No. The AI service falls back to a default value when this variable is not set.
...
The AI service ranks agent candidates using a blend score that combines a base relevance score with tool coverage and platform or type bonuses. Tool coverage measures the fraction of an agent's required tools that the user already has connected, so agents where the user has all needed integrations rank higher than those requiring new connections.

The `SEARCH_TOOL_WEIGHT` environment variable controls how much tool coverage contributes to the final ranking. When the user has selected preferred tools, agents are split into two groups: those that use at least one selected tool, or declare no tools, fill slots first, and the rest only backfill remaining slots to avoid empty results.
...
The web app supports USD as the only currency for credits and bounty payments. Currency pickers have been removed from the credits top-up modal, settings, and profile sections.

<ParamField body="ENABLED_CURRENCIES" param-type="string[]" required="true" show-location="false">

**Purpose:** Define which currencies users can select for credits and payments.

**Type:** array of strings.

**Explicit value:** `["USD"]`.

**Required:** Yes. The value is fixed in application configuration.
...
This configuration previously supported USD, INR, EUR, and GBP. It has been reduced to USD only.
