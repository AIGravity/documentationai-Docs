...
This repository currently relies far more on static configuration than on environment-driven configuration. The AI service reads two environment variables for the agent verification prewarm cache configuration. No `.env` files are defined in the repository; the variables use defaults when not set. The implementation does not include an app-level feature flag system.
...
The AI service reads two environment variables for the agent verification prewarm cache. Both have defaults and are optional.

<Callout kind="info">

No `.env` files are defined in the repository. The AI service reads two environment variables via standard OS environment access, with defaults applied when the variables are not set.
...
<ParamField body="VERIFICATION_PREWARM_CACHE_TTL" param-type="integer" required="false" show-location="false">

**Purpose:** Set the time-to-live for the agent verification prewarm cache (Gate 4 behaviour results), in seconds.

**Type:** integer.

**Default:** `1800` (30 minutes).

**Required:** No. The AI service uses the default when this variable is not set.


</ParamField>

<ParamField body="VERIFICATION_PREWARM_CACHE_SIZE" param-type="integer" required="false" show-location="false">

**Purpose:** Set the maximum number of entries in the agent verification prewarm cache.
