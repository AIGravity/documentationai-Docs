...
This page reflects the current implementation in `apps/web`. The shell includes routing, layout, styling, error handling, local scripts, and Docker packaging. Beyond the admin billing dashboard, it does not yet include broader app-specific screens, session handling, or non-billing workflow UI.
...
- no domain-specific navigation beyond the admin billing dashboard
- no business workflow screens beyond admin billing

The two external links in the resources list open in a new tab. The welcome screen remains a starter-style landing page, while the admin billing dashboard provides the first product-specific UI with API-driven data.
...
The frontend shell includes an admin billing dashboard with backend data integration, but does not yet include authentication UI, session handling, or broader product screens. Beyond billing, it remains a starter scaffold with routing, layout, styling, error handling, and packaging.


</Callout>

That constraint is important when planning the next layer of frontend work. Additional product pages, loaders, actions, shared app chrome, and broader API integration still need to be introduced on top of this shell.
