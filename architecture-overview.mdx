...
The practical architecture today is best understood as a deployment-ready foundation rather than a feature-complete platform. Each service starts, builds, and ships independently, but there are now a persistence layer via Supabase for bounty funding, JWT-based authentication via `SupabaseAuthGuard` on bounty endpoints, and Razorpay payment integration for the pay-to-post bounty flow, while cross-service request flow between the API and AI service remains unimplemented.
...
The root module lives at `apps/api/src/app.module.ts`. In addition to the health controller, the API now includes bounty controllers, a `BountyFundingService` for Razorpay pay-to-post order creation and verification, a `SupabaseService` for persistence, and a `SupabaseAuthGuard` for JWT-based authentication on protected routes.

The health controller at `apps/api/src/health/health.controller.ts` exposes `GET /health` with a static payload. Bounty endpoints are exposed through a bounties controller with `POST /api/bounties`, `GET /api/bounties/by-session/{sessionId}`, `POST /api/bounties/{id}/pay/order`, and `POST /api/bounties/{id}/pay/verify`. A Razorpay webhook controller handles `POST /api/webhooks/razorpay`. The bounty funding flow uses a service layer (`BountyFundingService`), a DTO layer (`VerifyBountyOrderDto` with `class-validator`), and a Supabase-backed persistence layer (`bounty_orders` and `bounties` tables).

The bounty funding feature adds the API's first domain module with controller, service, DTO, and persistence layers. The health endpoint remains a standalone controller outside that module structure.
...
The API entry point is `apps/api/src/main.ts`. Route controllers include the health controller and bounty/payment controllers.

Implemented route behavior today:

- `GET http://localhost:3001/health`
- `POST /api/bounties` (create bounty, requires auth)
- `GET /api/bounties/by-session/{sessionId}` (get bounty by session, requires auth)
- `POST /api/bounties/{id}/pay/order` (create Razorpay order, requires auth)
- `POST /api/bounties/{id}/pay/verify` (verify payment, requires auth)
...
Inside those boundaries, the internal module graph is still mostly absent. The bounty funding module follows a controller-to-service-to-persistence pattern (`BountiesController` → `BountyFundingService` → `SupabaseService`), while the health endpoint remains a standalone controller. The FastAPI app does not yet split routes into routers or separate business logic into services. The frontend does not yet separate route modules from API clients, domain packages, or shared contracts.
...
Bounty funding is persisted via Supabase. The `bounty_orders` table and additional columns on the `bounties` table (`payment_status`, `funded_at`) are managed through SQL migrations. Other services and domain areas do not yet have persistence.
...
Bounty endpoints are protected by `SupabaseAuthGuard`, which validates a JWT bearer token and extracts the user ID. The health and Razorpay webhook endpoints remain public. Other services do not yet have authentication.
...
- Razorpay payment integration for bounty pay-to-post funding in the API service
- Supabase as the persistence layer for `bounty_orders` and `bounties` tables
...
The strongest part of the current architecture is operational structure: app boundaries, runtime separation, containerization, and release automation. The bounty funding feature adds the first domain module with persistence, authentication, and payment integration. Other domain areas remain at an early implementation stage.
...
Bounty funding has answered some application-level questions — domain logic lives in feature modules, state is stored in Supabase, and authentication is handled via JWT. What remains unanswered is how contracts are shared across services, how the API and AI service collaborate, and how other domain areas beyond bounty funding will be structured.
