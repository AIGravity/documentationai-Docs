...
This repository currently relies far more on static configuration than on environment-driven configuration. The inspected implementation does not define any `.env` files and does not read application environment variables in source code. Runtime billing behavior is controlled through a database-stored pricing configuration system rather than environment variables.
...
The repository implements a database-stored pricing configuration system that controls billing behavior at runtime. Settings are stored as key-value pairs in a `pricing_config` table and are read by the billing service when processing agent run charges.

<Callout kind="info">

These settings differ from environment variables: they live in the application database and can be changed without redeploying the API service. A post-deploy verification script checks that all expected keys are present with the correct values after each deployment.
...
The following keys are currently in production (Alpha release):

<ParamField body="marketplace_exact_settlement" param-type="string" required="true" show-location="false">

**Purpose:** Toggle exact settlement for marketplace run charges. When enabled (`1`), charges are computed from measured LLM costs. When disabled, charges use a cost-plus formula based on a flat fee and markup multiplier.

**Type:** string (numeric flag).

**Explicit value:** `1`.

**Required:** Yes. Controls the primary charge computation path for marketplace runs.


</ParamField>

<ParamField body="chat_billing_enabled" param-type="string" required="true" show-location="false">

**Purpose:** Enable or disable billing for chat-based agent runs.

**Type:** string (numeric flag).

**Explicit value:** `1`.

**Required:** Yes. When disabled, chat runs are not charged.


</ParamField>

<ParamField body="platform_share_pct" param-type="string" required="true" show-location="false">

**Purpose:** Define the platform's share of marketplace run charges as a percentage. The remaining percentage goes to the agent creator.

**Type:** string (integer percentage).

**Explicit value:** `80`.

**Required:** Yes. Determines the revenue split between the platform and creators.


</ParamField>

<ParamField body="base_run_fee" param-type="string" required="true" show-location="false">

**Purpose:** Set the flat fee in credits charged for every agent run, applied before any LLM cost markup.

**Type:** string (integer credits).

**Explicit value:** `1`.

**Required:** Yes. Forms the baseline charge for all runs.


</ParamField>

<ParamField body="llm_markup_multiplier" param-type="string" required="true" show-location="false">

**Purpose:** Set the multiplier applied to raw LLM cost when computing the cost-plus charge amount.

**Type:** string (integer multiplier).

**Explicit value:** `8`.

**Required:** Yes. Used only on the cost-plus settlement path.


</ParamField>

<ParamField body="creator_share_basis" param-type="string" required="true" show-location="false">

**Purpose:** Determine how creator earnings are calculated. When set to `gross`, the creator share is computed from the full charge amount. When set to `margin`, the creator share is computed from the difference between the charge and the raw LLM cost.

**Type:** string (enum).

**Explicit value:** `gross`.

**Required:** Yes. Controls the basis for creator revenue splitting.


</ParamField>

<ParamField body="marketplace_structural_pricing" param-type="string" required="true" show-location="false">

**Purpose:** Enable structural pricing estimation for marketplace runs when exact telemetry data is unavailable. When enabled (`1`), the system estimates charges from run structure rather than requiring measured costs.

**Type:** string (numeric flag).

**Explicit value:** `1`.

**Required:** Yes. Provides a fallback pricing path when telemetry is missing.


</ParamField>

<ParamField body="marketplace_exact_settlement_allowlist" param-type="string" required="false" show-location="false">

**Purpose:** Restrict exact settlement to specific runs. When empty, exact settlement applies to all runs.

**Type:** string (allowlist).

**Explicit value:** empty (applies to all runs).

**Required:** No. Currently unrestrictive.
