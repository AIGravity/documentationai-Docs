...
The Dockerfile does not define a non-root user, a HEALTHCHECK instruction, environment-specific configuration, or deployment-time setup. It packages and runs the current minimal service as-is.
...
On `dev`, the workflow generates a development version using the base version, a timestamp, and a short SHA. It tags the repository, publishes the same image with the generated version tag and `dev-latest`, and also adds a SHA tag.
