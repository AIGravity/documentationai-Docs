...
The API service is a minimal NestJS foundation app in `apps/api`. It boots a single Nest application, registers one `HealthController`, enables CORS for one local frontend origin, and listens on a fixed port. The service now includes an admin billing module with authentication guards, a service layer, and a Supabase-backed Postgres persistence layer managed through SQL migrations. DTOs, OpenAPI/Swagger setup, and environment-based configuration are not yet implemented.

<Callout kind="info">

The service exposes a public health endpoint at `GET /health` and a set of admin-protected billing endpoints under `/api/admin/billing/`. The module graph now includes the admin billing module alongside the health controller.
...
`AppModule` is the full application root, and it wires the health controller and an admin billing module. The admin billing module contributes a controller, a service layer, and authentication guards.
...
- `AppModule` declares the health controller and imports the admin billing module
- `AppModule` imports the admin billing feature module

Runtime behavior now includes a health controller for service availability and an admin billing module that provides billing diagnostics, user lookup, and revenue reporting through a service layer backed by a Supabase Postgres database.
...
The module graph now includes a service layer for billing operations, but DTOs and validation pipes are not yet formalized. Request validation relies on inline checks rather than class-validator decorators.
...
The implementation boots a Nest service, answers a health check, provides admin billing diagnostics through a service layer backed by Postgres, builds with TypeScript, packages into a container, and publishes through CI. The next layer of work would need to resolve the current mismatches first, especially the fixed port configuration, Docker port exposure, and the likely built-entry path mismatch between `main` and `start`.